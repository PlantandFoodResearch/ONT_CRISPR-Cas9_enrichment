...
/software/bioinformatics/MUMmer-3.23/delta-filter -l 2000 -q -r Chr01 -i 90 -1 X2250_Canu_vs_GDv1.1 > Chr01_i90_X2250_canu_vsGDv1.1.delta
...
#This accounts for 7934 contigs of a total of 8664
 ...
 rep -v "Chr00" X2250_Canu_vs_GDv1.1.coords | awk '{if($6>=9700) print$11}' | sort -u | wc -l
#7934
...
do 
grep 'Chr'$j X2250_Canu_vs_GDv1.1.coords | awk '{if($6>=9700) print$11}' | sort -u > chr$j'_9700_coords.txt'
done
```
...
debug <- TRUE
for(i in ls(pattern="mumgp.filt.diag*")) {
   for (j in 1:17) 
  tiffFile <- paste0(i, ".tiff")
  chr <- paste0("Chr",j)
  if(debug) print(tiffFile)
  ## Now create tiff
  tiff('tiffFile", width=18, height=20, units="cm",
       pointsize=8, compression="lzw", bg="white", res=600)
  
 ggplot(tiffFile, aes(x=rs, xend=re, y=qs, yend=qe, colour=strand)) +
  geom_segment(show.legend=FALSE) + geom_point(alpha=0.09) + theme_bw() + theme(strip.text.y.left = element_text(angle = 0))+ facet_grid(qid~., scales='free', space='free', switch='both') +
  guides(colour=guide_legend(override.aes=list(alpha=1))) + 
  theme(strip.text.y=element_text(angle=0, size=4),
        strip.text.x=element_text(size=2),
        ...
        panel.spacing=unit(0, 'cm'),panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
panel.border = element_blank()) +
  xlab('chr') + ylab('assembly') + scale_colour_brewer(palette='Set1')
...
```{bash, engine.opts='-l'}
#Calculate the length of the all contigs minus those in Chr00 to see if they would be enough to get to the haploid genome

# number of countings without countings those on Chr00
grep -v "Chr00" i90_delta_9700.txt | awk '{print$2} | sort -u | wc -l 

#4913 countig one line of header


grep -v "Chr00" i90_delta_9700.txt |  awk '{print$2}' | sort -u > Rfiltered_contigs_i90_9700.txt

# remove "" from previous file 
sed 's/"tig/tig/' Rfiltered_contigs_i90_9700.txt | sed 's/"//' > mummerplot_contigs_i90_9700.txt

#Then I opened with nano editor and removed the header

grep -f mummerplot_contigs_i90_9700.txt ../Canu_X2250_raw_promethium.contigs.fasta | grep ">" > final_mummerplot_len_contigs_vs_GD_align_9700.txt

Then I opened in excel and summed up column 2 (lenght of contigs) = 1249,254,555 ~ 1,2 Gb
Those contigs with >=100 reads per contig account for 715 Mb so it is plenty to be sure we have a complete haploid genome at least

```
#Write the table to a text file for filtering by Chromosome so later the pictures can be easier to process
```{bash, engine.opts='-l'}


cd /workspace/hrpelg/X2250_ONT/ANU_X2250/Guppy_basecalled_reads/04.canu/nucmer
delta=/workspace/hrpelg/X2250_ONT/ANU_X2250/Guppy_basecalled_reads/04.canu/nucmer/i90_delta_9700.txt

for t in 01 02 03 04 05 06 07 08 09 10 11 12 13 14 15 16 17 
do
grep Chr$t $delta > Chr$t'i90_delta_9700.txt'
done
